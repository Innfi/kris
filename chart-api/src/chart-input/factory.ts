...
): Readonly<LoadChartInputBase> => {
  const { chartType, symbol, interval } = reqInput;

  switch (chartType) {
  case 'intraday':
    return new LoadChartInputIntraday(symbol, interval as string);
  case 'daily':
    return new LoadChartInputDaily(symbol);
  case 'weekly':
    return new LoadChartInputWeekly(symbol);
  case 'monthly':
    return new LoadChartInputMonthly(symbol);
  default:
